...
// Constants used for the charging station
#define NUM_CHARGING_PORTS 3 // k
#define AVAILABLE_THRESHOLD 1 // if <30% of charging ports are available, station is heavily in use
...
//Function Prototypes
int run_base_station(MPI_Comm world_comm, MPI_Comm comm);
...
int main(int argc, char **argv)
{
    ...
    if (rank == 0){
        ...
        */
        ...
        // TODO: prompt user for other parameters (e.g. number of charging ports per station, threshold for heavily in use, threshold for free, number of iterations, etc.)

        // Broadcast the user-specified dimensions to setup the cartesian grid
        MPI_Bcast(dims, ndims, MPI_INT, 0, MPI_COMM_WORLD);

        // Start Simulation
        run_base_station( MPI_COMM_WORLD, commEVCS );
    }
    //Charging Station
    else{
        // Receive the broadcasted dimensions to setup the cartesian grid
        MPI_Bcast(dims, ndims, MPI_INT, 0, MPI_COMM_WORLD);

        availability_arr = (int*) malloc(NUM_CHARGING_PORTS * sizeof(int));
...
int run_charging_station(MPI_Comm world_comm, MPI_Comm comm, int *dims)
/*
This is the main function to simulate a charging station.
*/
{
    ...
    pthread_t tid[NUM_CHARGING_PORTS];
    pthread_t messenger_id;
    struct charging_port_t args[NUM_CHARGING_PORTS];
    ...
    while(!end){
        ...
        if(availability_count <= AVAILABLE_THRESHOLD){
        ...
        else if(availability_count >= FREE_THRESHOLD && reported){
...
void* runBaseMessenger(void *pArg)
/*
This is the function that is execucted by the messenger thread of the base station.
*/
{
...
void* runCSMessenger(void *pArg)
/*
This is the function that is execucted by the messenger thread of the charging stations.
*/
{
...
void* runChargingPort(void *pArg){
/* Function that simulates the use of a charging port. Updates its availability to the charging station via the availability array.
Input:
    pArg[0]: rank of the charging port (used to set a unique seed for the random number generator)
    pArg[1]: location of the charging port in the availability array 
*/
...
int stationLogFile(char *pFileName, struct report_t report, int* neighbours_available, int *other_neighbours, int log_type){
/* Function invoked by the charging station to log the new report entry into a file.*/
    FILE *fp;
    fp = fopen(pFileName, "a");
    if(fp == NULL){
        printf("Error opening file\n");
...
int find_neighbours_available(int* availability_list, int left, int right, int top, int bottom, MPI_Comm comm){
/*
Function to find the number of neighbouring charging stations that are available.
*/

...
S1 2023 FIT3081 Take Home Assessment - Program File for Experiment 1
"""
import numpy as np
import cv2
...
class Neural_Network:
    ...
    def Read_Files(self, labels):
        ...
        for label in labels:
            ...
            #Applying vertical and horizontal shifts
            horizontal = np.roll(G, 1, axis=0)
            training_files.append(np.array(horizontal).flatten())

            vertical = np.roll(G, 1, axis=1)
            training_files.append(np.array(vertical).flatten())
    ...
    def Saving_Weights_Bias(self,filename="DiongChenXi_Q2_Weights"):
        """
...
def Load_Weights_Bias(filename="DiongChenXi_Q2_Weights.npz"):

...
class Neural_Network:
    ...
    def Read_Files(self, labels):
        ...
        for label in labels:
            ...
            #Resize the image from 784 pixels to 49 pixels
            G = resize_to_49(G)
    ...
    def Saving_Weights_Bias(self,filename="DiongChenXi_Exp2_Weights"):
        """
...
def Load_Weights_Bias(filename="DiongChenXi_Exp2_Weights.npz"):
...
def resize_to_49(original):
    #784 = 28 x 28 and we want to resize to 49 = 7 x 7
    #Scan the image with a 4 x 4 window, and select the max value (max pooling)
    resized = np.zeros((7, 7))
    for i in range(0, 28, 4):
        for j in range(0, 28, 4):
            resized[int(i/4), int(j/4)] = np.max(original[i:i+2, j:j+2])
    return resized
...
if __name__ == "__main__":
    ...
    for test in testing_dataset:
        ...
        #Resize the image from 784 to 49 pixels
        G = resize_to_49(G)
